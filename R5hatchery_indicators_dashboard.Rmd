...
shinyApp(
  ...
  server = function(input, output, session) {
    # Reactive values
    subset_data_reactive<-reactive({ # Reactive expression for subset data
      indicators %>%
        filter(
    ...
    output$plot <- renderPlot({
      ...
      current_plot <- ggplot(subset_data_reactive(), aes(x = Brood_Year, y = value,color=species)) +
        ...
        #ggtitle(subset_data_reactive()$population[1])+
        ggtitle("shit")+
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(file) {
        ...
        write.csv(subset_data_reactive(), file, row.names = F)
      }
    )
