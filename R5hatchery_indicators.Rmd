...
pacman ::p_load(jsonlite, dplyr, RODBC, curl,odbc,DBI,tidyverse,janitor,fuzzyjoin,ggplot2,lubridate,kableExtra)
```
...
```{r results='hide',warning=FALSE,message=FALSE}
results<-readxl::read_excel("data/LCR_Unique_Spawns_v2.xlsx")%>%
  ...
  mutate(brood_origin=ifelse(mark_type == "Unmarked", "Wild", "Hatchery"), #start by assuming unclipped = Wild
         ...
         brood_origin=ifelse(mark_type == "Unmarked" & Wire_Collected_flag == 1 & name=="COWLITZ SALMON HATCHERY" & species_description%in%c("COHO","STEELHEAD"), "Wild",brood_origin), #wire positive unmarked coho and steelhead from upper basin are wild
